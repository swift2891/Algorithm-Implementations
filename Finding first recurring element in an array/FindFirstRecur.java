...
public class FindFirstRecur{
    ...
    // Two finger/pointer algorithm to merge two sorted arrays:    
    ...
    //Check either side of an element and see if there are duplicates:
    ...
    public static int binarySearch(int[] originalSorted,int[] receivedArray, int key,int arrayOffset){
    	...
    	if(receivedArray.length<2){
    		...
    		System.out.println("\n"+"mid element match"+"\n");
            System.out.println("\n"+"Inside mid match"+"\n"+"mid: "+mid+" arrayOffset: "+arrayOffset+" key: "+key);
    		if(originalSorted[mid+arrayOffset-1]==key){
    				System.out.println("\n"+"Match 4 "+"\n");
    				return key;
   			}
            if(mid+arrayOffset+1<=originalSorted.length-1)
                if(originalSorted[mid+arrayOffset+1]==key)
                    return key;               
                else
                    return 0;     
